...
Debut.prototype._iterateDom = function iterateDom(to, start) {
  if (!this._lastDomElement) {
    this._lastDomElement = this.$.container.children()[0];
  }

  // TODO: Stop elements from making repeat calls
  ...
  if ($children.length > 0) {
    if (this._iterateDom(to, $children[0])) {
      return true;
    }
  }

  // console.log('d');
  return this._finishDomElement(start, to);
}

/**
 * @private
 */
Debut.prototype._finishDomElement = function finishDomElement(element, to) {
  var $element = $(element);
  ...
  if ($sibling.length > 0) {
    ...
    return this._iterateDom(to, $sibling[0]);
  }
  ...
  if ($parent.length > 0) {
    return this._iterateDom(to, $parent[0]);
  }
...
Debut.prototype._runDomHooks = function runDomHooks($element, end) {
  ...
  this._animationAdders.forEach(function (adder) {
    if (((adder.options.on === 'exit') && (end)) || ((adder.options.on !== 'exit') && (!end))) {
