...
!function(exports, global) {
    ...
    var $ = jQuery, animations = {
        appear: function(elem, context, params, callback) {
            1 === params.direction ? $(elem).css("visibility", "visible") : -1 === params.direction && $(elem).css("visibility", "hidden"), 
            callback();
        },
        slide: function(elem, context, params) {
            ...
            switch (params = $.extend({
                side: "left"
            }, params), $(elem).css("visibility", "visible"), params.side) {
              default:
              case "left":
    ...
    }, Animation = function(fun, params) {
        this.params = params, this.run = function(elem, context, callback) {
    ...
    $.fn.getDOMOptions = function(template) {
        ...
        var addChildren = function() {
            ...
            if (options.anim && (elem.css("visibility", "hidden"), options.anim.params = $.extend({}, {
                direction: 1,
                duration: 500,
                easing: "in-out"
            }, options.anim.params), animationQueue.push(function() {
                options.anim.run(elem, context, function() {});
            })), $(this).children().each(addChildren), options.endExitAll) {
                var queue = [];
                ...
                duration: 500,
                easing: "in-out"
            }, options.endExit.params), animationQueue.push(function() {
            ...
            elem = $(elem), options.anim && (options.anim.params = $.extend({}, {
                direction: 1,
                duration: 500,
                easing: "in-out"
            }, options.anim.params), queue.push(function() {
                options.anim.run(elem, context, function() {});
            })), $(this).children().each(function(key, val) {
                childrenExit(queue, val);
            });
        };
        container.children().each(addChildren), $(window).keypress(function() {
            var fun = animationQueue.shift();
            if ("function" == typeof fun) fun(); else {
                if ("object" != typeof fun) throw new Error("Bad animation thingie");
