/*! oliver-and-swan 2014-02-09 */
!function(exports, global) {
    function Animation(fun, params) {
        this.params = params, this.fun = fun, this.start = "onstep", this.delay = 0, this._elem = null, 
    ...
    var $ = jQuery, animations = {
        ...
        var containerHeight, containerWidth, masterWidth, masterHeight, container = $('<div class="presentation-container"></div>'), slideMaster = outerContainer, domOptions = $(outerContainer).getDOMOptions(slideMasterOptions), $this = this;
        $(outerContainer).addClass("slide-master"), options = $.extend({}, domOptions, options), 
        containerHeight = options.containerHeight, containerWidth = containerHeight * options.aspectRatio, 
        container.height(containerHeight), container.width(containerWidth), container.css("transform-origin", "0 0"), 
        ...
        }), $(outerContainer).children("*").appendTo(container), container.appendTo(outerContainer), 
        this.outerContainer = outerContainer, this.innerContainer = container;
        var addChildren = function() {
            ...
            elem = $(elem), console.log(elem), options.anim && !options.exit && (options.exit = $.extend(!0, new Animation(), options.anim)), 
        ...
        this.innerContainer.children().each(addChildren), $(window).keydown(function(e) {
            ...
            if (fun = animationQueue[$this.index], type = fun.constructor.name, "Animation" !== type) throw new Error("Only Animation objects can be in here");
            fun.run($this, reverse), reverse || ($this.index += 1), $this.index in animationQueue && "withprevious" === animationQueue[$this.index].start && setTimeout(function() {
              ...
              case "string":
                switch (type) {
                  ...
                  case "animation":
                    ...
                    if (parsed) params = eval("({" + parsed[2] + ")"), anim = parsed[1]; else if (parsed = attr ? attr.match(/^{([\s\S]*)}$/m) : null) {
